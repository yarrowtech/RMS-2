...
export default function AdminSettings({ departmentMode = false }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setBusinessProfile(next.business_profile || {});
  ...
  const saveBusinessProfile = async () => { try { setBusy(true); setError(""); const response = await api("/business-profile", { method: "PATCH", body: JSON.stringify(businessProfile) }); setMessage(response.message); setBusinessProfile(response.business_profile || businessProfile); } catch (e) { setError(e.message); } finally { setBusy(false); } };
  ...
  return <div className="min-h-full bg-slate-50 p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-5xl space-y-5">
    ...
    <div className="flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-sm">{tabs.map(([key, Icon, label]) => <button key={key} onClick={() => setTab(key)} className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold ${tab === key ? "bg-indigo-600 text-white" : "text-slate-600 hover:bg-slate-50"}`}>{React.createElement(Icon,{className:"h-4 w-4"})}{label}</button>)}</div>
    ...
    {tab === "organisation" && <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-black text-slate-900">Business name & contact</h2><p className="mt-1 text-sm text-slate-500">HQ-only. This is the name shown to vendors, job workers and RMS — including a single-store Basic plan.</p><div className="mt-5 grid gap-4 sm:grid-cols-2"><Label value={businessProfile.company_name || ""} onChange={e => setBusinessProfile({ ...businessProfile, company_name: e.target.value })}>Store / business name</Label><Label value={businessProfile.phone || ""} onChange={e => setBusinessProfile({ ...businessProfile, phone: e.target.value })}>Phone</Label><Label value={businessProfile.city || ""} onChange={e => setBusinessProfile({ ...businessProfile, city: e.target.value })}>City</Label><Label value={businessProfile.state || ""} onChange={e => setBusinessProfile({ ...businessProfile, state: e.target.value })}>State</Label></div><button disabled={busy} onClick={saveBusinessProfile} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-black text-white disabled:opacity-60"><Building2 className="h-4 w-4"/>Save business name & contact</button></section>}
