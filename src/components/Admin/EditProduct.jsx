...
function Toast({ msg, type, onClose }) {
  ...
  return (
    <div
      className={`fixed bottom-7 right-7 z-[9999] flex max-w-[340px] items-center gap-2 rounded-2xl border px-4 py-3 text-sm font-medium shadow-2xl ${
        type === "ok"
  ...
  return (
    <div className="mb-0">
      ...
      <div className="relative">
        <span className="pointer-events-none absolute left-3 top-1/2 z-[1] -translate-y-1/2 font-mono text-xs text-slate-400">
          ₹
        </span>
        <input
...
function IRow({ k, v }) {
  return (
    <div className="flex items-center justify-between gap-3 border-b border-slate-100 py-2.5 last:border-b-0">
      <span className="whitespace-nowrap text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-400">
        {k}
      </span>
      <div className="break-all text-right font-mono text-xs text-slate-500">
        {v || "—"}
      </div>
    </div>
  );
  ...
  if (loading) {
    return (
      <div className={embedded ? "min-h-[320px]" : "min-h-screen bg-slate-50"}>
        {!embedded && (
          <div className="sticky top-0 z-30 flex h-[60px] items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur sm:px-6">
            <div className="flex items-center gap-3">
              <div className="h-8 w-[72px] animate-pulse rounded-lg bg-slate-200" />
              <div className="h-4 w-[110px] animate-pulse rounded bg-slate-200" />
            </div>
            <div className="h-9 w-[120px] animate-pulse rounded-xl bg-slate-200" />
          </div>
        )}
    ...
    <div className={embedded ? "w-full bg-transparent" : "min-h-screen bg-slate-50"}>
      {!embedded && (
        <header className="sticky top-0 z-30 flex h-[60px] items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur sm:px-6">
          <div className="flex items-center gap-3 sm:gap-4">
            <button
              className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-500 transition hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-700"
              onClick={goBack}
            >
              <svg
                width="14"
            ...
            <span className="hidden text-sm font-bold uppercase tracking-[0.08em] text-slate-500 sm:inline">
              Edit Product
            </span>
          ...
          <div className="flex items-center gap-2">
            <button
              className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-500 transition hover:border-red-200 hover:bg-red-50 hover:text-red-700"
              onClick={goBack}
            >
              Discard
            </button>
            ...
            <button
              className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-700 to-indigo-500 px-5 py-2 text-sm font-semibold text-white shadow-sm transition hover:-translate-y-[1px] hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50"
              onClick={handleSave}
              disabled={saving}
            >
              {saving ? (
      ...
      {embedded && (
        <div className="mb-5 flex flex-col gap-4 rounded-2xl border border-indigo-200 bg-gradient-to-r from-indigo-50 via-white to-blue-50 p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-600 transition hover:border-red-200 hover:bg-red-50 hover:text-red-700"
              onClick={goBack}
            >
              Back
            </button>
      ...
      <div
        className={`mx-auto grid max-w-7xl grid-cols-1 gap-5 ${
          embedded ? "" : "px-4 py-6 sm:px-6"
        } lg:grid-cols-[1fr_340px]`}
      >
        <div className="flex min-w-0 flex-col gap-5">
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
              <div className="flex h-[30px] w-[30px] items-center justify-center rounded-lg border border-indigo-200 bg-indigo-50 text-sm">
                ✏️
              </div>
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
                ...
                <span className="flex items-center text-[11px] font-bold uppercase tracking-[0.12em] text-slate-500">
                  ...
                  <span className="ml-2 inline-flex min-w-[20px] items-center justify-center rounded-full bg-indigo-50 px-2 py-0.5 text-[10px] font-bold text-indigo-700">
                    {variants.length}
                  </span>
                </span>

                <span
                  className={`ml-auto rounded-full border px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.1em] ${
                    vType === "size_color"
                ...
                <table className="w-full min-w-[760px] border-collapse text-xs">
                  <thead className="sticky top-0 z-[1] bg-white">
                    <tr className="border-b border-slate-200">
                      <th className="px-3 py-2 text-left text-[9px] font-bold uppercase tracking-[0.12em] text-slate-400">
                        Variant
                      </th>
                      <th className="px-3 py-2 text-left text-[9px] font-bold uppercase tracking-[0.12em] text-slate-400">
                        Cost ₹
                      ...
                      >
                        <td className="px-2 py-2 align-middle">
                          <div className="flex flex-col gap-1">
                            {v.size_label && (
                              <span className="inline-flex max-w-[120px] items-center rounded-md border border-slate-200 bg-slate-100 px-2 py-1 text-[11px] text-slate-500">
                            ...
                            {v.color && (
                              <span className="inline-flex max-w-[120px] items-center gap-1 rounded-md border border-slate-200 bg-slate-100 px-2 py-1 text-[11px] text-slate-500">
                                <span
                                  className="h-[9px] w-[9px] shrink-0 rounded-full border border-black/10"
                        ...
                        <td className="px-2 py-2">
                          <input
                            ...
                            step="0.01"
                            value={v.cost_price ?? ""}
                            onChange={(e) => patchV(i, "cost_price", e.target.value)}
                          />
                        </td>
                        ...
                        <td className="px-2 py-2">
                          <input
                            ...
                            step="0.01"
                            value={v.mrp ?? ""}
                            onChange={(e) => patchV(i, "mrp", e.target.value)}
                          />
                        </td>
                        ...
                        <td className="px-2 py-2">
                          <input
                            ...
                            onChange={(e) => patchV(i, "selling_price", e.target.value)}
                          />
                        </td>

                        <td className="px-2 py-2">
                          <input
                            className="min-w-[56px] w-full rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 font-mono text-xs text-slate-900 outline-none transition focus:border-indigo-500"
                            type="number"
                            min="0"
                            value={v.stock ?? ""}
                            onChange={(e) => patchV(i, "stock", e.target.value)}
                          />
                        </td>
        ...
        <div className="flex min-w-0 flex-col gap-5">
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
              ...
              <span className="flex items-center text-[11px] font-bold uppercase tracking-[0.12em] text-slate-500">
                ...
                {keepImgs.length + newFiles.length > 0 && (
                  <span className="ml-2 inline-flex min-w-[20px] items-center justify-center rounded-full bg-indigo-50 px-2 py-0.5 text-[10px] font-bold text-indigo-700">
                    {keepImgs.length + newFiles.length}
                  </span>
                )}
              </span>
            </div>
            ...
            <div className="p-5">
                ...
                <div className="mb-4 grid grid-cols-3 gap-2.5">
                  {keepImgs.map((url) => (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        className="absolute right-1 top-1 flex h-[22px] w-[22px] items-center justify-center rounded-full bg-red-500 text-[10px] text-white opacity-0 transition group-hover:opacity-100"
                        onClick={() => setKeepImgs((p) => p.filter((u) => u !== url))}
                      >
                        ✕
                      </button>
                    </div>
              ...
              {newFiles.length > 0 && (
                <>
                  <p className="mb-2 text-[9px] font-bold uppercase tracking-[0.1em] text-indigo-700">
                    New — pending upload
                  </p>
                  <div className="mb-4 grid grid-cols-3 gap-2.5">
                    {newFiles.map((f, i) => (
                      <div
                        key={i}
                        className="relative aspect-square overflow-hidden rounded-xl border border-indigo-300 bg-indigo-50"
                      >
                        <img
                        ...
                        />
                        <span className="absolute bottom-1 left-1 rounded bg-indigo-600 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-[0.08em] text-white">
                          New
                        </span>
                        <button
                          type="button"
                          className="absolute right-1 top-1 flex h-[22px] w-[22px] items-center justify-center rounded-full bg-red-500 text-[10px] text-white"
              ...
              >
                ...
                <p className="text-sm leading-6 text-slate-400">
                  <span className="font-semibold text-indigo-700">Click to upload</span> or
                  drag & drop
                  <br />
                  PNG · JPG · WEBP
                </p>
