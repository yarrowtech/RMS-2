...
const statusStyle = {
  ...
  AWAITING_QC: "bg-fuchsia-50 text-fuchsia-700 ring-fuchsia-200",
  REWORK_PENDING: "bg-rose-50 text-rose-700 ring-rose-200",
  COMPLETED: "bg-emerald-50 text-emerald-700 ring-emerald-200",
};

// The finished-goods QC gate: a receipt's output sits here until a
// retailer-side user accepts/rejects/reworks it — see qc_job_work_receipt
// in job_work_routes.py. Never assume any un-QC'd output is sellable.
function pendingQcOutput(order) {
  return (order.outputs || []).find((o) => o.qc_status === "pending") || null;
}
...
export default function ProductionJobWork() {
  ...
  return (
    <main className="min-h-full bg-[radial-gradient(circle_at_top_right,_rgba(99,102,241,0.16),_transparent_30%),linear-gradient(135deg,_#f8fafc_0%,_#eef2ff_46%,_#ecfeff_100%)] p-4 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex gap-6">
          ...
          <div className="min-w-0 flex-1">
            ...
            {activeStep === "design-collab" && <ProductionDesignCollaboration />}
          </div>
        </div>
      </div>
      ...
      {modal?.type === "qc" && <RunQcModal order={modal.order} output={modal.output} onClose={closeModal} onSaved={async (message) => { closeModal(); showNotice(message); await refresh(); }} setError={setError} />}
...
function ProductionDesignCollaboration() {
  const [data, setData] = useState({ projects: [], queries: [], change_requests: [] });
  const [form, setForm] = useState({ project_id: "", category: "Construction feasibility", description: "", priority: "MEDIUM" });
  const [error, setError] = useState(""); const [loading, setLoading] = useState(true);
  const call = async (path, options = {}) => { const response = await fetch(`${API_BASE_URL}/api/design-pattern${path}`, { ...options, headers: { ...authHeaders(), ...(options.headers || {}) } }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.detail || "Unable to update Design collaboration."); return body; };
  const load = async () => { setLoading(true); try { setData(await call("/collaboration")); setError(""); } catch (e) { setError(e.message); } finally { setLoading(false); } };
  // The collaboration panel owns this one-time initial refresh.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => { load(); }, []);
  const submit = async (event) => { event.preventDefault(); try { await call("/queries", { method: "POST", body: JSON.stringify(form) }); setForm({ project_id: "", category: "Construction feasibility", description: "", priority: "MEDIUM" }); await load(); } catch (e) { setError(e.message); } };
  const decide = async (change, status) => { const decision_note = window.prompt(`${status === "ACCEPTED" ? "Acceptance" : "Rejection"} note`); if (decision_note === null) return; try { await call(`/change-requests/${change.id}`, { method: "PATCH", body: JSON.stringify({ status, decision_note }) }); await load(); } catch (e) { setError(e.message); } };
  return <section className="overflow-hidden rounded-3xl border border-fuchsia-100 bg-white shadow-xl"><header className="border-b bg-gradient-to-r from-fuchsia-50 to-white p-5"><p className="text-xs font-black uppercase tracking-widest text-fuchsia-600">Cross-department control</p><h2 className="mt-1 text-xl font-black text-slate-900">Design queries & change requests</h2><p className="text-sm text-slate-500">Raise technical questions against a design and formally review post-release changes.</p></header>
    <div className="grid gap-6 p-5 xl:grid-cols-2"><div><h3 className="mb-3 font-black text-slate-800">Raise query to Design</h3><form onSubmit={submit} className="space-y-3 rounded-2xl border bg-slate-50 p-4"><Field label="Design"><select required value={form.project_id} onChange={(e) => setForm({ ...form, project_id: e.target.value })}><option value="">Select design</option>{data.projects.map((p) => <option key={p.id} value={p.id}>{p.design_no} · {p.style_name}</option>)}</select></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="Category"><select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>{["Measurement clarification", "Pattern file missing", "Fabric specification", "Artwork placement", "Trim unavailable", "Consumption concern", "Construction feasibility", "Other"].map((x) => <option key={x}>{x}</option>)}</select></Field><Field label="Priority"><select value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })}><option>LOW</option><option>MEDIUM</option><option>HIGH</option><option>URGENT</option></select></Field></div><Field label="Question / production issue"><textarea required rows="3" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></Field><button className="rounded-xl bg-fuchsia-600 px-4 py-2.5 text-sm font-bold text-white">Send to Design</button></form>{error && <p className="mt-3 rounded-xl bg-rose-50 p-3 text-sm font-bold text-rose-700">{error}</p>}</div>
      <div><h3 className="mb-3 font-black text-slate-800">Production decisions</h3><div className="mb-4 space-y-2">{data.projects.filter(p=>!(p.approvals||[]).some(a=>a.type==="PRODUCTION_FEASIBILITY"&&a.decision==="APPROVED")).slice(0,6).map(p=><div key={p.id} className="flex items-center justify-between rounded-xl border p-3"><span className="text-sm font-bold">{p.design_no} · {p.style_name}</span><button onClick={async()=>{const note=window.prompt("Feasibility note")||"Feasible for production";try{await call(`/projects/${p.id}/approval`,{method:"POST",body:JSON.stringify({approval_type:"PRODUCTION_FEASIBILITY",decision:"APPROVED",note})});await load();}catch(e){setError(e.message);}}} className="rounded-lg bg-violet-600 px-3 py-2 text-xs font-bold text-white">Mark feasible</button></div>)}</div><div className="space-y-3">{data.change_requests.filter((c) => c.status === "PENDING_PRODUCTION_REVIEW").length ? data.change_requests.filter((c) => c.status === "PENDING_PRODUCTION_REVIEW").map((c) => <article key={c.id} className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><p className="font-black text-amber-950">{c.change_no} · {c.design_no}</p><p className="mt-1 text-sm text-amber-900">{c.reason}</p><p className="mt-2 text-xs text-amber-800">Material: {c.material_impact || "None"} · Cost: {c.cost_impact || "None"} · Delivery: {c.delivery_impact || "None"}</p><div className="mt-3 flex gap-2"><button onClick={() => decide(c, "ACCEPTED")} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white">Accept change</button><button onClick={() => decide(c, "REJECTED")} className="rounded-lg bg-rose-600 px-3 py-2 text-xs font-bold text-white">Reject</button></div></article>) : <p className="rounded-2xl border border-dashed p-8 text-center text-sm text-slate-400">No changes awaiting Production review.</p>}</div></div>
    </div><div className="border-t p-5"><div className="flex items-center justify-between"><h3 className="font-black text-slate-800">Query feed</h3><button onClick={load} className="text-xs font-bold text-fuchsia-700">Refresh</button></div>{loading ? <p className="py-6 text-center text-sm text-slate-400">Loading…</p> : <div className="mt-3 space-y-2">{data.queries.slice(0, 20).map((q) => <div key={q.id} className="rounded-xl border p-3 text-sm"><div className="flex flex-wrap items-center gap-2"><b>{q.query_no} · {q.design_no}</b><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold">{String(q.status).replaceAll("_", " ")}</span><span className="text-xs font-bold text-rose-600">{q.priority}</span></div><p className="mt-1 text-slate-600">{q.description}</p>{q.response && <p className="mt-1 text-xs text-emerald-700"><b>Design response:</b> {q.response}</p>}</div>)}</div>}</div>
  </section>;
}

function RunQcModal({ order, output, onClose, onSaved, setError }) {
  const receivedQty = Number(output.received_qty || output.quantity || 0);
  const [accepted, setAccepted] = useState(String(receivedQty));
  const [rejected, setRejected] = useState("0");
  const [rework, setRework] = useState("0");
  const [notes, setNotes] = useState("");
  const [saving, setSaving] = useState(false);
  const remaining = receivedQty - (Number(accepted) || 0) - (Number(rejected) || 0) - (Number(rework) || 0);
  const submit = async (event) => {
    event.preventDefault();
    setSaving(true);
    try {
      const result = await request(`/orders/${order.id}/receipts/${output.receipt_id}/qc`, {
        method: "POST",
        body: JSON.stringify({ accepted_qty: Number(accepted) || 0, rejected_qty: Number(rejected) || 0, rework_qty: Number(rework) || 0, qc_notes: notes }),
      });
      await onSaved(result.message);
    } catch (err) { setError(err.message); } finally { setSaving(false); }
  };
  return <Modal title={`Run QC · ${order.order_no}`} onClose={onClose}><form onSubmit={submit} className="p-6">
    <div className="mb-5 rounded-2xl bg-fuchsia-50 p-4 text-sm text-fuchsia-900">
      <b>{receivedQty} {output.product}</b> received from <b>{order.job_worker_name}</b> — not yet in sellable stock.
      Only what you mark <b>Accepted</b> below is added to inventory now. <b>Rejected</b> is a permanent write-off. <b>Rework</b> goes back to the job worker — record another receipt on this order once the corrected batch comes back.
    </div>
    <div className="grid grid-cols-3 gap-3">
      <Field label="Accepted"><input min="0" step="any" type="number" value={accepted} onChange={(e) => setAccepted(e.target.value)} /></Field>
      <Field label="Rejected"><input min="0" step="any" type="number" value={rejected} onChange={(e) => setRejected(e.target.value)} /></Field>
      <Field label="Rework"><input min="0" step="any" type="number" value={rework} onChange={(e) => setRework(e.target.value)} /></Field>
    </div>
    <p className={`mt-2 text-xs font-bold ${remaining === 0 ? "text-emerald-600" : "text-amber-600"}`}>
      {remaining === 0 ? "Fully accounted for." : `${remaining} of ${receivedQty} not yet assigned.`}
    </p>
    <div className="mt-4"><Field label="QC notes"><input value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Stitching defect, size mismatch, colour issue…" /></Field></div>
    <div className="mt-6 flex justify-end gap-3">
      <button type="button" onClick={onClose} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold text-slate-600">Cancel</button>
      <button disabled={saving || remaining !== 0} className="rounded-xl bg-fuchsia-600 px-5 py-2.5 text-sm font-bold text-white disabled:opacity-60">{saving ? "Saving…" : "Confirm QC"}</button>
    </div>
  </form></Modal>;
}
