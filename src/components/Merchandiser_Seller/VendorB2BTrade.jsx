...
export default function VendorB2BTrade({ onNavigate }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [partnerData, rfqData, orderData, invoiceData, financeData] = await Promise.all([
        request("/partners"), request("/rfqs"), request("/orders"), request("/invoices"), request("/finance-ledger"),
      ]);
      setPartners(partnerData.data || []); setRfqs(rfqData.data || []); setOrders(orderData.data || []); setInvoices(invoiceData.data || []); setFinanceLedger(financeData || null);
  ...
  const createRfq = (event) => {
    ...
    run(async () => {
      ...
      setRfqForm(emptyRfq);
    }, "RFQ (Request for Quotation) sent to your connected supplier.");
  };
  ...
  const downloadFile = async (path, filename) => {
    try { const response = await fetch(`${API_BASE_URL}/api/vendor-b2b${path}`, { headers: authHeaders() }); if (!response.ok) { const data = await response.json().catch(() => ({})); throw new Error(data.detail || "Download failed"); } const blob = await response.blob(); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = filename; link.click(); URL.revokeObjectURL(url); } catch (err) { setError(err.message); }
  };
  const dispatch = (order) => {
    const expected = window.prompt("Expected delivery date (YYYY-MM-DD):"); if (!expected) return;
    const transporter = window.prompt("Transporter / courier name (optional):") || "";
    const tracking = window.prompt("Tracking / AWB number (optional):") || "";
    run(() => request(`/orders/${order._id}/dispatch`, { method: "PUT", body: JSON.stringify({ expected_delivery_date: expected, transporter_name: transporter, tracking_number: tracking }) }), "Dispatch saved. Download the delivery challan below.");
  };
  const payInvoice = (invoice) => {
    const amount = window.prompt(`Payment amount (outstanding: ${invoice.currency} ${invoice.balance_due ?? invoice.amount}):`); if (!amount) return;
    const reference = window.prompt("Payment reference / UTR (optional):") || "";
    run(() => request(`/invoices/${invoice._id}/payments`, { method: "POST", body: JSON.stringify({ amount, reference_no: reference }) }), "B2B payment recorded and outstanding balance updated.");
    ...
    <section className="rounded-3xl border border-teal-100 bg-gradient-to-br from-teal-50 via-white to-indigo-50 p-5 shadow-sm sm:p-6">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        {[
          ...
          ["2", "Send RFQ (Request for Quotation)", "Choose an accepted supplier and describe quantity, specification and target price."],
