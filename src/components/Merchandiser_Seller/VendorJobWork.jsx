...
function TechPackSnapshot({ pack }) {
  return <div className="mt-2 rounded-lg border border-violet-100 bg-violet-50 p-2 text-xs text-slate-600">
    <div className="flex flex-wrap items-center justify-between gap-2"><p className="font-black text-violet-800">{pack.tech_pack_no || "Tech pack"} · {pack.version || "v1"}</p><button type="button" onClick={() => { void downloadTechPackPdf(pack); }} className="rounded-lg border border-violet-200 bg-white px-2 py-1 text-[10px] font-black text-violet-700 hover:bg-violet-100">Download Tech Pack PDF</button></div>
    {(pack.theme_name || pack.collection || pack.designer_name) && <p className="text-slate-500">{[pack.theme_name, pack.collection, pack.designer_name].filter(Boolean).join(" Â· ")}</p>}
    ...
    {pack.process_allowances?.some((row) => ["ANY", "EXTERNAL"].includes(String(row.worker_scope || "ANY").toUpperCase())) && <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-2"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-black text-amber-900">Approved manual process allowances</p><span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-black text-emerald-700">{String(pack.allowance_approval?.status || "NOT_REQUIRED").replaceAll("_", " ")}</span></div><p className="mt-1 text-[10px] text-amber-800">Follow these values exactly; they do not increase the issued quantity.</p><div className="mt-1 divide-y divide-amber-100">{pack.process_allowances.filter((row) => ["ANY", "EXTERNAL"].includes(String(row.worker_scope || "ANY").toUpperCase())).map((row,index)=><div key={String(row.process)+"-"+index} className="py-1.5"><p><b>{row.process} · {row.allowance_type || "Manual allowance"}:</b> {row.value} {String(row.unit||"").replaceAll("_"," / ")} · {row.basis || "per garment"}</p><p className="text-[10px] text-slate-500">Fabric: {row.fabric_reference || "all"}{row.reason ? " · Reason: "+row.reason : ""}</p></div>)}</div></div>}
    ...
    {(pack.artwork_width_cm || pack.artwork_height_cm || pack.artwork_placement) && (
      <p className="mt-1"><b>Artwork:</b> {[pack.artwork_width_cm && `${pack.artwork_width_cm}cm w`, pack.artwork_height_cm && `${pack.artwork_height_cm}cm h`, pack.artwork_placement].filter(Boolean).join(" Â· ")}</p>
    )}
...
{!acknowledged && order.status !== "DRAFT" && <div className="mt-3 space-y-2 rounded-lg border border-teal-200 bg-white p-3">
  ...
  <button onClick={() => acknowledge(order)} disabled={saving === `ack-${order.id}`} className="mt-1 w-full rounded-lg bg-teal-600 px-3 py-2.5 text-sm font-bold text-white disabled:opacity-60">{saving === `ack-${order.id}` ? "Savingâ€¦" : "Acknowledge challan"}</button>
</div>}
