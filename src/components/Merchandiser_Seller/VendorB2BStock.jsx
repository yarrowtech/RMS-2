...
function B2BStockGuide() {
  const steps = [
    ["1", "Where it comes from", "Stock here comes only from completed Vendor B2B Trade orders with other vendors — receiving a material/goods order adds it automatically. Retailer POs, GRC and GRN never touch this ledger."],
    ["2", "On hand vs. average cost", "On hand is your current quantity of that item. Average cost is the weighted average price across every B2B receipt of it, used to value your stock — it updates as new receipts come in."],
    ["3", "Manual adjustments", "Use Stock out / consumed when material is used, damaged, or needs correcting down; Stock in / correction for an upward correction. A reason is always required — it's saved permanently."],
    ["4", "View movement", "Click View movement on any item to see its full history — every B2B receipt and every manual adjustment, with date, quantity and value."],
  ];
  return (
    <details open className="overflow-hidden rounded-2xl border border-cyan-100 bg-gradient-to-br from-white to-cyan-50 shadow-sm">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4">
        <div className="flex items-center gap-3">
          <span className="grid h-9 w-9 place-items-center rounded-xl bg-cyan-100 text-sm font-black text-cyan-700">i</span>
          <div><p className="text-sm font-black text-slate-900">How the B2B Stock Ledger works</p><p className="mt-0.5 text-xs text-slate-500">Why it's separate from retailer stock, and what you can adjust</p></div>
        </div>
        <span className="rounded-full border border-cyan-200 bg-white px-3 py-1 text-[11px] font-bold text-cyan-700">Show / hide guide</span>
      </summary>
      <div className="border-t border-cyan-100 px-5 pb-5 pt-4">
        <div className="mb-4 rounded-xl bg-teal-50 px-3.5 py-3 text-xs leading-6 text-teal-800"><strong>Important:</strong> This ledger tracks only stock you've received through Vendor B2B Trade with other vendors. It has no connection to any retailer's stock, GRC or GRN records — even if you supply the same item to both.</div>
        <div className="grid gap-2.5 sm:grid-cols-2">
          {steps.map(([number, title, text]) => (
            <div key={number} className="flex gap-2.5 rounded-xl border border-cyan-100 bg-white p-3">
              <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-cyan-600 text-[11px] font-black text-white">{number}</span>
...
export default function VendorB2BStock({ onNavigate }) {
  ...
  return <div className="mx-auto max-w-7xl space-y-5">
    ...
    <B2BStockGuide />
    <TaxProfileBanner onNavigate={onNavigate} context="your B2B stock records" />
