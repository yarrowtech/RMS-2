...
function SelectField({ label, value, onChange, options, hint }) {
  return <label className="block"><span className="mb-1.5 block text-xs font-extrabold uppercase tracking-[0.12em] text-slate-500">{label}</span><select value={value ?? ""} onChange={(event) => onChange?.(event.target.value)} className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-800 outline-none transition focus:border-teal-500 focus:ring-4 focus:ring-teal-100">{options.map((o) => <option key={o || "_empty"} value={o}>{o || "Select..."}</option>)}</select>{hint && <span className="mt-1.5 block text-[11px] leading-4 text-slate-400">{hint}</span>}</label>;
}
...
export default function MsellerSettings({ onNavigate = () => {} }) {
  const [profile, setProfile] = useState({ name: "", email: "", contactMobile: "", address: "", city: "", website: "", pan: "", gstin: "", gstCategory: "", gstState: "", legalEntityType: "" });
  const [notifications, setNotifications] = useState(DEFAULT_NOTIFICATIONS);
  const [orderPreferences, setOrderPreferences] = useState(DEFAULT_ORDER_PREFERENCES);
  const [subscription, setSubscription] = useState(null);
  const [kyb, setKyb] = useState({ legal_name: "", business_address: "", bank_account_holder: "", bank_name: "", ifsc: "", account_number: "", account_last4: "", gst_certificate_url: "", pan_document_url: "", cancelled_cheque_url: "", aadhar_number: "", aadhar_last4: "", aadhar_document_url: "" });
  ...
  const uploadKybDocument = async (documentType, file) => {
    const fieldByType = { gst_certificate: "gst_certificate_url", pan_document: "pan_document_url", cancelled_cheque: "cancelled_cheque_url", aadhar_document: "aadhar_document_url" };
  ...
  return <div className="mx-auto max-w-6xl space-y-5 pb-8">
    ...
    <Card icon={ShieldCheck} title="Tax & registration" subtitle="Not collected at signup — add these once you're ready. Used on commercial documents with retailers." action={<button type="button" onClick={() => save("profile")} disabled={saving !== ""} className="inline-flex h-10 items-center gap-2 rounded-xl bg-teal-600 px-3.5 text-xs font-extrabold text-white transition hover:bg-teal-700 disabled:opacity-50">{saving === "profile" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Save</button>}>
      <div className="grid gap-4 sm:grid-cols-2">
        <SelectField label="Business entity type" value={profile.legalEntityType} options={BUSINESS_ENTITY_TYPES} onChange={(value) => setProfile((p) => ({ ...p, legalEntityType: value }))} hint="Sole Proprietorship needs an Aadhaar in KYB below; other entity types don't." />
