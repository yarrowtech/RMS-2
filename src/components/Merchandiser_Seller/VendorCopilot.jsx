...
const GUIDES = {
  ...
  settings: { title: "Vendor Settings & KYB", summary: "Keep business, tax and verification details current so retailers can safely trade with you.", steps: ["Save PAN and GSTIN in Tax & registration first.", "Open Vendor Verification (KYB) and enter legal business, bank, IFSC and document links.", "Submit once; each retailer reviews it separately. Only the last four account digits are displayed.", "If a retailer requests changes, update the details and submit again before a new PO can be assigned."], next: "purchase-order", cta: "Open Purchase Orders" },
...
export default function VendorCopilot({ activeTab, onNavigate }) {
  ...
  const localAsk = () => {
    ...
    else if (query.includes("kyb") || query.includes("bank") || query.includes("gst") || query.includes("pan") || query.includes("verify") || query.includes("verification")) setReply("Open Settings, then Vendor Verification (KYB). First save PAN and GSTIN. Enter legal business name, bank name, account holder name, IFSC, account number, and secure GST, PAN and cancelled-cheque document links. RMS displays only the last four account digits. Submit for each retailer finance review; after the KYB deadline, a retailer cannot assign a new PO until they mark you Verified.");
  ...
  return <div className="fixed bottom-4 right-4 z-[70] sm:bottom-6 sm:right-6">
    {open && <section className="mb-3 flex max-h-[calc(100dvh-6.5rem)] w-[calc(100vw-2rem)] max-w-sm flex-col overflow-hidden rounded-3xl border border-violet-100 bg-white shadow-[0_24px_70px_rgba(76,29,149,0.24)] sm:max-h-[min(44rem,calc(100dvh-7rem))]">
