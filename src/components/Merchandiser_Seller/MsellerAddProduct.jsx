...
// ─── Empty shapes ─────────────────────────────────────────────────────────────
const emptyPricing = () => ({
  cost_price: "", mrp: "", selling_price: "", quantity: "", unit: "pcs",
});
...
function Label({ children, small = false }) {
  return (
    <label
      className={`mb-2 block font-semibold uppercase tracking-wide text-slate-900 ${
...
function Input({ mono = false, className = "", ...props }) {
  return (
    <input
      {...props}
      className={`w-full min-w-0 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-500 hover:border-slate-500 focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-100 ${
        mono ? "font-mono" : ""
      } ${className}`}
    />
  );
...
function Select({ className = "", ...props }) {
  return (
    <select
      {...props}
      className={`w-full min-w-0 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition hover:border-slate-500 focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-100 ${className}`}
    />
  );
}
...
function Textarea({ className = "", ...props }) {
  return (
    <textarea
      {...props}
      className={`w-full min-w-0 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm leading-7 text-slate-900 outline-none transition placeholder:text-slate-500 hover:border-slate-500 focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-100 ${className}`}
    />
  );
}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1">
        <span className="text-[9px] font-bold uppercase tracking-[0.9px] opacity-70">Profit</span>
      ...
      {margin !== null && (
        <>
          ...
          <div className="flex items-center gap-1">
            <span className="text-[9px] font-bold uppercase tracking-[0.9px] opacity-70">Margin</span>
      ...
      {discount !== null && (
        <>
          <span className="opacity-40">·</span>
          <div className="flex items-center gap-1">
            <span className="text-[9px] font-bold uppercase tracking-[0.9px] opacity-70">Disc off MRP</span>
      ...
      {total !== null && total > 0 && (
        <>
          <span className="opacity-40">·</span>
          <div className="flex items-center gap-1">
            <span className="text-[9px] font-bold uppercase tracking-[0.9px] opacity-70">Stock Value</span>
...
function StatCards({ cp, sp, qty, compact = false }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      <div className={`rounded-xl border ${pad} ${isLoss ? "border-red-200 bg-red-50" : "border-emerald-200 bg-emerald-50"}`}>
        <p className={`mb-1 ${title} font-bold uppercase tracking-[1px] ${isLoss ? "text-red-700" : "text-emerald-700"}`}>
          {isLoss ? "Loss" : "Margin"}
        </p>
        <p className={`font-mono ${val} font-semibold ${isLoss ? "text-red-700" : "text-emerald-700"}`}>
          ₹{fmt(profit)}
        </p>
      </div>
      <div className={`rounded-xl border border-violet-200 bg-violet-50 ${pad}`}>
        <p className={`mb-1 ${title} font-bold uppercase tracking-[1px] text-violet-700`}>Total</p>
        <p className={`font-mono ${val} font-semibold text-violet-700`}>₹{fmt(total)}</p>
      </div>
    </div>
  );
}
...
function ColorGrid({ selected = [], onToggle, compact = false }) {
  ...
  return (
    <div className={`flex flex-wrap gap-1.5 overflow-y-auto rounded-xl border border-slate-200 bg-slate-50 ${compact ? "max-h-28 p-3" : "max-h-44 p-3"}`}>
      {WEB_SAFE_COLORS.map((c) => (
        <button
          key={c} type="button" title={c} onClick={() => onToggle(c)}
          className={`shrink-0 rounded-md border-2 transition hover:scale-110 ${compact ? "h-4 w-4" : "h-5 w-5"} ${isOn(c) ? "border-slate-900 ring-2 ring-black/10" : "border-transparent"}`}
  ...
  return (
    <div className="mb-3 rounded-2xl border border-indigo-200 bg-indigo-50/50 p-5">
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <span className="h-5 w-5 rounded-md border-2 border-black/10" style={{ backgroundColor: variant.color }} />
        <span className="font-mono text-xs text-slate-500">{variant.color}</span>
      </div>
      <div className="mb-3 grid grid-cols-1 gap-3 md:grid-cols-3">
        <div>
          <Label>Cost Price (₹)</Label>
          <Input mono type="number" min="0" step="0.01" placeholder="0.00" value={variant.cost_price}
            onKeyDown={preventInvalidKeys}
            onChange={(e) => onChange("cost_price", sanitizeNum(e.target.value, ""))} />
        </div>
        <div>
          <Label>MRP (₹)</Label>
          <Input mono type="number" min="0" step="0.01" placeholder="0.00" value={variant.mrp}
            onKeyDown={preventInvalidKeys}
            onChange={(e) => { const v = sanitizeNum(e.target.value, ""); onChange("mrp", v); if (!variant.selling_price) onChange("selling_price", v); }} />
        </div>
        <div>
          <Label>Selling Price (₹)</Label>
          <Input mono type="number" min="0" step="0.01" placeholder="Auto from MRP" value={variant.selling_price}
            onKeyDown={preventInvalidKeys}
      ...
      <div className="mb-3 grid grid-cols-1 gap-3 md:grid-cols-2">
        <div>
          ...
          <Input mono type="number" min="0" step="1" placeholder="0" value={variant.quantity}
            onKeyDown={preventInvalidKeys}
            onChange={(e) => onChange("quantity", sanitizeInt(e.target.value, ""))} />
        </div>
        <div>
          <Label>Unit</Label>
          <Select value={variant.unit} onChange={(e) => onChange("unit", e.target.value)}>
  ...
  return (
    <div className="min-w-0 rounded-xl border border-slate-200 bg-white p-4">
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <span className="rounded border border-indigo-200 bg-indigo-50 px-2 py-0.5 font-mono text-[10px] font-semibold text-indigo-700">{size}</span>
        {customSize && <span className="rounded border border-slate-200 bg-slate-100 px-2 py-0.5 font-mono text-[10px] text-slate-500">{customSize}</span>}
        <span className="inline-block h-[13px] w-[13px] shrink-0 rounded border border-black/10" style={{ backgroundColor: colorObj.color }} />
        <span className="font-mono text-[10px] text-slate-500">{colorObj.color}</span>
      </div>
      <div className="mb-2 grid grid-cols-1 gap-2 sm:grid-cols-3">
        <div>
          <Label small>Cost (₹)</Label>
          <Input mono type="number" min="0" step="0.01" placeholder="0.00" value={colorObj.cost_price}
            onKeyDown={preventInvalidKeys} className="px-3 py-2 text-[13px]"
            onChange={(e) => onChange("cost_price", sanitizeNum(e.target.value, ""))} />
        </div>
        <div>
          <Label small>MRP (₹)</Label>
          <Input mono type="number" min="0" step="0.01" placeholder="0.00" value={colorObj.mrp}
            onKeyDown={preventInvalidKeys} className="px-3 py-2 text-[13px]"
            onChange={(e) => { const v = sanitizeNum(e.target.value, ""); onChange("mrp", v); if (!colorObj.selling_price) onChange("selling_price", v); }} />
        </div>
        <div>
          <Label small>Selling (₹)</Label>
          <Input mono type="number" min="0" step="0.01" placeholder="Auto" value={colorObj.selling_price}
            onKeyDown={preventInvalidKeys} className="px-3 py-2 text-[13px]"
            onChange={(e) => onChange("selling_price", sanitizeNum(e.target.value, ""))} />
        </div>
      </div>
      ...
      <div className="mb-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
        <div>
          ...
          <Input mono type="number" min="0" step="1" placeholder="0" value={colorObj.quantity}
            onKeyDown={preventInvalidKeys} className="px-3 py-2 text-[13px]"
            onChange={(e) => onChange("quantity", sanitizeInt(e.target.value, ""))} />
        </div>
        <div>
          <Label small>Unit</Label>
          <Select value={colorObj.unit} onChange={(e) => onChange("unit", e.target.value)} className="px-3 py-2 text-[13px]">
            {UNIT_OPTIONS.map((u) => <option key={u.value} value={u.value}>{u.label}</option>)}
          </Select>
        </div>
      </div>
      {showStats && <StatCards cp={colorObj.cost_price} sp={colorObj.selling_price} qty={colorObj.quantity} compact />}
    </div>
  );
}
    ...
    <div className="mb-4 rounded-2xl border border-indigo-200 bg-indigo-50/50 p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-lg border border-indigo-200 bg-indigo-100 px-4 py-1 font-mono text-sm font-semibold text-indigo-700">{sizeVariant.size}</span>
          <span className="text-xs text-slate-500">{sizeVariant.colors.length} color{sizeVariant.colors.length !== 1 ? "s" : ""}</span>
        </div>
        <button type="button" className="rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-medium text-red-600 transition hover:bg-red-100" onClick={onRemove}>
          ✕ Remove
        </button>
      </div>
      <div className="mb-4">
        <Label>Custom Size Value</Label>
        <Input type="text" placeholder="e.g. 30 / 32 / Free Size" value={sizeVariant.custom_size} onChange={(e) => onCustomSize(e.target.value)} />
      </div>
      <div className="mb-4">
      ...
      {sizeVariant.colors.length > 0 && (
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          {sizeVariant.colors.map((cObj) => (
            <SizeColorMiniCard key={cObj.color} size={sizeVariant.size} customSize={sizeVariant.custom_size}
              colorObj={cObj} onChange={(field, value) => onColorField(cObj.color, field, value)} />
          ))}
        </div>
      )}
    </div>
...
export default function AddProduct({ onSuccess, onClose } = {}) {
  ...
  const handleChange = (i, f, v) => {
    ...
    if (f === "requires_expiry" && !v) { u[i].expiry_date = ""; u[i].shelf_life_days = ""; }
    setProducts(u);
  };
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-indigo-300 via-purple-400 to-indigo-500 px-4 pb-28 pt-6 sm:px-5 lg:px-6">
      <div className="mx-auto w-full max-w-7xl rounded-3xl border border-white/30 bg-white/20 p-4 shadow-[0_30px_80px_rgba(0,0,0,0.2)] backdrop-blur-xl">

        {/* Header */}
       <div className="mb-6 flex flex-col gap-4 rounded-2xl border border-white/30 bg-white/70 p-5 shadow-sm backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {onClose && (
              <button
                type="button"
                onClick={onClose}
                className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-500 transition hover:border-red-200 hover:bg-red-50 hover:text-red-700"
              >
                ✕ Close
              </button>
            )}
        ...
        <form onSubmit={handleSubmit}>
          {products.map((form, idx) => (
            <div key={idx} className="relative mb-5 overflow-hidden rounded-2xl border border-white/30 bg-white/70 p-4 shadow-lg backdrop-blur-xl sm:p-6">
              ...
              <div className="mb-4">
                ...
                <Input type="text" placeholder="e.g. Classic Polo Shirt" value={form.product_name} required
              ...
              {/* ── TAX & COMPLIANCE ─────────────────────────────────────── */}
              <Sec label="Tax & Compliance" />
              <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  <Label>HSN Code</Label>
                  <Input type="text" placeholder="e.g. 6109 (optional)" value={form.hsn_code} maxLength={8}
                    onChange={(e) => handleChange(idx, "hsn_code", e.target.value)} />
                  <p className="mt-1 text-[11px] text-slate-400">4–8 digit code for GST classification</p>
                </div>
                <div>
                  <Label>GST Rate</Label>
                  <Select value={form.gst_rate} onChange={(e) => handleChange(idx, "gst_rate", e.target.value)}>
                    {GST_SLABS.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
                  </Select>
                  {form.gst_rate && (
                    <div className="mt-2 inline-flex flex-wrap items-center gap-2 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1.5">
                      <span className="font-mono text-[11px] font-semibold text-indigo-700">CGST {(Number(form.gst_rate) / 2).toFixed(1)}%</span>
                      <span className="text-indigo-300 text-xs">+</span>
                      <span className="font-mono text-[11px] font-semibold text-indigo-700">SGST {(Number(form.gst_rate) / 2).toFixed(1)}%</span>
                      <span className="text-indigo-300 text-xs">·</span>
                      <span className="font-mono text-[11px] font-semibold text-indigo-500">IGST {form.gst_rate}%</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="mb-4">
                <label className="flex cursor-pointer items-start gap-3">
                  ...
                  <div>
                    <span className="text-[11px] font-bold uppercase tracking-wide text-slate-900">Requires Expiry Date</span>
                    <p className="mt-0.5 text-[11px] text-slate-400">
                      Enable for food, beverages, medicines, cosmetics. Batch-level expiry is also captured during GRN stock-in.
                    </p>
                  </div>
                </label>
                {form.requires_expiry && (
                  <div className="mt-3 grid grid-cols-1 gap-4 pl-7 md:grid-cols-2">
                    <div>
                      <Label>Initial Expiry Date *</Label>
                      <Input type="date" value={form.expiry_date} min={today}
                        onChange={(e) => handleChange(idx, "expiry_date", e.target.value)} />
                      <p className="mt-1 text-[11px] text-slate-400">Expiry date for the current stock being added</p>
                    </div>
                    <div>
                      <Label>Default Shelf Life (days)</Label>
                      <Input mono type="number" min="1" placeholder="e.g. 365" value={form.shelf_life_days}
                        onKeyDown={preventInvalidKeys}
                        onChange={(e) => handleChange(idx, "shelf_life_days", sanitizeInt(e.target.value, ""))} />
                      <p className="mt-1 text-[11px] text-slate-400">Used to auto-suggest expiry on future GRN entries</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              {!form.has_variants && (
                <>
                  ...
                  <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
                    <div>
                      <Label>Cost Price (₹)</Label>
                      <Input mono type="number" min="0" step="0.01" placeholder="0.00" value={form.cost_price}
                        onKeyDown={preventInvalidKeys}
                        onChange={(e) => handleChange(idx, "cost_price", sanitizeNum(e.target.value, ""))} />
                    </div>
                    <div>
                      <Label>MRP (₹)</Label>
                      <Input mono type="number" min="0" step="0.01" placeholder="0.00" value={form.mrp}
                        onKeyDown={preventInvalidKeys}
                        onChange={(e) => handleChange(idx, "mrp", sanitizeNum(e.target.value, ""))} />
                    </div>
                    <div>
                      <Label>Selling Price (₹)</Label>
                      <Input mono type="number" min="0" step="0.01" placeholder="Auto from MRP" value={form.selling_price}
                        onKeyDown={preventInvalidKeys}
                  ...
                  <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                      <Label>Quantity</Label>
                      <Input mono type="number" min="0" step="1" placeholder="0" value={form.quantity}
                        onKeyDown={preventInvalidKeys}
                        onChange={(e) => handleChange(idx, "quantity", sanitizeInt(e.target.value, ""))} />
                    </div>
                    <div>
                      <Label>Unit</Label>
                      <Select value={form.unit} onChange={(e) => handleChange(idx, "unit", e.target.value)}>
                        {UNIT_OPTIONS.map((u) => <option key={u.value} value={u.value}>{u.label}</option>)}
                      </Select>
                    </div>
                  </div>
              ...
              <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-center">
                <label className="flex cursor-pointer items-center gap-2 text-sm font-medium text-slate-900">
                  <input type="checkbox" checked={form.has_variants}
                    onChange={(e) => { handleChange(idx, "has_variants", e.target.checked); handleChange(idx, "variants", []); }}
                ...
                {form.has_variants && (
                  <Select
                    ...
                    onChange={(e) => { handleChange(idx, "variant_type", e.target.value); handleChange(idx, "variants", []); }}
                  >
                ...
                <div className="mb-4">
                  <Label>
                    ...
                    {form.variants.length > 0 && (
                      <span className="ml-2 font-mono font-normal text-indigo-700">({form.variants.length} selected)</span>
                    )}
                  </Label>
              ...
              {form.has_variants && form.variant_type === "size_color" && (
                <div className="mb-4">
                  ...
                  <div className="mb-4 flex flex-wrap gap-2">
                    {SIZE_OPTIONS.map((sz) => (
                      <button key={sz} type="button" onClick={() => toggleSize(idx, sz)}
              ...
              <div className="mb-4">
                ...
                <Textarea rows={3} placeholder="Short product description…" value={form.description}
                  onChange={(e) => handleChange(idx, "description", e.target.value)} />
              </div>
              <div className="mb-4">
                <Label>Specification</Label>
                <Textarea rows={3} placeholder="Material, dimensions, care instructions…" value={form.specification}
              ...
              <div className="mb-4">
                <div className="relative overflow-hidden rounded-2xl border-2 border-dashed border-indigo-300 bg-indigo-50 px-4 py-8 text-center transition hover:border-indigo-500 hover:bg-indigo-100">
                  ...
                  <p className="text-sm text-slate-500">
                    <strong className="font-semibold text-indigo-700">Click to browse</strong>{" "}
                    or drag &amp; drop images here
                  </p>
                  <p className="mt-1 text-xs text-slate-500">PNG · JPG · WEBP — multiple files allowed</p>
                </div>
                {form.images.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {form.images.map((f, i) => (
                      <span key={i} className="max-w-full break-all rounded-md border border-indigo-200 bg-indigo-50 px-3 py-1 text-xs font-medium text-indigo-700">
                        ✓ {f.name}
                      </span>
                    ))}
                  </div>
          ...
          {/* ── STICKY FOOTER ────────────────────────────────────────────── */}
          <div className="sticky bottom-3 z-10 mt-7 flex flex-col gap-3 rounded-2xl border border-white/30 bg-white/75 p-4 backdrop-blur-xl sm:flex-row">
