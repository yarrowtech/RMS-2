...
import {
  ...
  Check, Search, RefreshCw, LogIn, BarChart3, UserPlus, AlertCircle, UploadCloud, Store, Trash2, Pencil, ListChecks,
} from "lucide-react";
...
function AssignTaskModal({ staff, onClose, onAssigned }) {
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      await hrFetch("/staff-tasks", { method: "POST", body: JSON.stringify(form) });
      onAssigned();
      onClose();
    } catch (e) { window.alert(e.message || "Could not assign task."); }
  ...
  return (
    <div className="fixed inset-0 z-[999] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="bg-gradient-to-r from-teal-600 to-indigo-700 px-6 py-5 flex items-center justify-between">
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <select className="w-full h-11 px-3 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-teal-400"
          ...
          <div>
            ...
            <input className="w-full h-11 px-3 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-teal-400"
              value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} placeholder="e.g. Complete joining formalities" />
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-widest mb-1.5">Details</label>
            <textarea className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-teal-400"
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="date" className="w-full h-11 px-3 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-teal-400"
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 transition">Cancel</button>
          <button onClick={submit} disabled={saving} className="flex-1 py-2.5 bg-gradient-to-r from-teal-600 to-indigo-700 text-white rounded-xl text-sm font-bold hover:opacity-90 transition disabled:opacity-50">
...
function TasksView() {
  ...
  // One-time permission probe, independent of which tab is showing and of
  // whether there happen to be zero tasks or zero assignable staff yet.
  useEffect(() => {
    hrFetch("/staff-tasks").then(() => setCanManage(true)).catch(() => setCanManage(false));
    hrFetch("/staff-tasks/assignable-staff").then((res) => setStaff(res.data || [])).catch(() => {});
  }, []);

  const load = useCallback(async (activeView) => {
    setLoading(true);
    try {
      const t = await hrFetch(activeView === "mine" ? "/staff-tasks/mine" : "/staff-tasks");
      setTasks(t.data || []);
    } catch { setTasks([]); }
  ...
  const updateStatus = async (task, status) => {
    try { await hrFetch(`/staff-tasks/${task.id}`, { method: "PATCH", body: JSON.stringify({ status }) }); load(view); }
    catch (e) { window.alert(e.message || "Could not update task."); }
  };

  const remove = async (task) => {
    try { await hrFetch(`/staff-tasks/${task.id}`, { method: "DELETE" }); load(view); }
    catch (e) { window.alert(e.message || "Could not delete task."); }
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {view === "mine" ? "Tasks assigned to you" : "Tasks you've assigned at your store"}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {canManage && (
            <button onClick={() => setShowAssign(true)} className="inline-flex items-center gap-2 px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-xl text-sm font-bold transition">
      ...
      <div className="inline-flex rounded-xl border border-slate-200 bg-slate-50 p-1">
        <button onClick={() => setView("mine")}
          className={`px-4 py-1.5 rounded-lg text-xs font-bold transition ${view === "mine" ? "bg-white text-teal-700 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
          My Tasks
        </button>
        {canManage && (
          <button onClick={() => setView("managed")}
            className={`px-4 py-1.5 rounded-lg text-xs font-bold transition ${view === "managed" ? "bg-white text-teal-700 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
            Assigned by me
          </button>
        )}
      </div>
      ...
      {showAssign && <AssignTaskModal staff={staff} onClose={() => setShowAssign(false)} onAssigned={() => { setView("managed"); load("managed"); }} />}
    </div>
  );
}
