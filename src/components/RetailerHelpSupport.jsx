...
import { ArrowUpCircle, Building2, CheckCircle2, ChevronDown, ChevronUp, CircleHelp, FileText, Loader2, MessageCircle, Plus, RefreshCw, Send } from "lucide-react";
...
export default function RetailerHelpSupport() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setForm({ category: "technical", subject: "", description: "", attachment_url: "" });
  ...
  return <main className="mx-auto max-w-6xl space-y-5 p-4 sm:p-6">
    ...
    {openForm && <form onSubmit={submit} className="rounded-3xl border border-indigo-100 bg-white p-5 shadow-sm sm:p-6">
      ...
      <label className="mt-4 block text-sm font-bold text-slate-700">Screenshot or document link <span className="font-normal text-slate-400">(optional)</span><input type="url" value={form.attachment_url} onChange={(event) => setForm((value) => ({ ...value, attachment_url: event.target.value }))} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm" placeholder="https://..." /></label>
