...
export default function PublicVendorCatalogue() {
  ...
  const addToCart = (item, size = "", color = "") => {
    ...
    setCart((current) => {
      ...
      const cap = Math.max(0, Number(item.available_to_order ?? item.stock ?? 0));
      if (existing) {
        return current.map((line) => line.key === key ? { ...line, quantity: Math.min(cap || Infinity, Number(line.quantity) + 1) } : line);
      }
      return [...current, {
        key,
  ...
  const updateQty = (key, nextQty) => setCart((current) => current.map((line) => {
    ...
    const minimum = Math.max(1, Number(line.moq) || 1);
  ...
  const submit = async () => {
    ...
    if (!buyer.name.trim() || !buyer.phone.trim()) { setError("Enter your name and mobile number."); return; }
    setSubmitting(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/catalogue/public/${vendorId}/orders`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ buyer, note, items: cart }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.detail || "Could not send this catalogue order.");
      setCart([]); setCheckoutOpen(false); setNote("");
      setSuccess(data.message || "Catalogue order request sent.");
    } catch (err) {
      setError(err.message || "Could not send this catalogue order.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return <div className="min-h-screen bg-[#071411] text-white">
    ...
    <main className="mx-auto max-w-5xl px-4 pb-28 pt-5">
        ...
        <div className="p-6 sm:p-8">
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-2 text-sm text-emerald-100">Browse, add to cart, and send your order request directly to this vendor.</p>
            </div>
          </div>
      ...
      <div className="mt-6 flex items-end justify-between gap-4">
        <div><h2 className="text-2xl font-black">Products</h2><p className="mt-1 text-sm text-slate-300">Fixed-price products available for direct catalogue order request.</p></div>
...
function ProductCard({ item, onAdd }) {
  ...
  return <article className="overflow-hidden rounded-3xl bg-white text-slate-950 shadow-xl shadow-black/20">
    ...
    <div className="p-4">
      ...
      <div className="mt-3 flex items-center justify-between text-xs font-bold text-slate-500"><span>MOQ {item.moq || 1}</span><span>{Math.floor(Number(item.available_to_order ?? item.stock ?? 0))} available</span></div>
