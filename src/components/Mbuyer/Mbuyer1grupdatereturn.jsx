...
export default function Mbuyer1GRUpdateReturn() {
  ...
  return <div className="h-full overflow-y-auto bg-slate-50 p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-6xl space-y-5">
    ...
    <PostGRNReturnPanel onCreated={load} />
...
function PostGRNReturnPanel({ onCreated }) {
  const [grns, setGrns] = useState([]); const [selectedId, setSelectedId] = useState("");
  const [forms, setForms] = useState({}); const [note, setNote] = useState("");
  const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [message, setMessage] = useState("");
  const loadGrns = useCallback(async () => {
    try { setBusy(true); setError(""); const response = await fetch(`${API}/eligible-grns`, { headers: auth(), cache: "no-store" }); const data = await response.json(); if (!response.ok) throw new Error(data.detail || "Could not load posted GRNs."); setGrns(data.data || []); }
    catch (requestError) { setError(requestError.message || "Could not load posted GRNs."); }
    finally { setBusy(false); }
  }, []);
  useEffect(() => { loadGrns(); }, [loadGrns]);
  const current = grns.find((row) => row.grn_id === selectedId);
  const choose = (id) => { setSelectedId(id); const row = grns.find((item) => item.grn_id === id); const next = {}; (row?.lines || []).forEach((line) => { next[line.grn_item_index] = { selected: false, quantity: "", reason: "Quality issue found after receipt" }; }); setForms(next); setNote(""); setMessage(""); };
  const update = (index, field, value) => setForms((currentForms) => ({ ...currentForms, [index]: { ...currentForms[index], [field]: value } }));
  const save = async () => {
    const lines = (current?.lines || []).filter((line) => forms[line.grn_item_index]?.selected).map((line) => ({ grn_item_index: line.grn_item_index, quantity: Number(forms[line.grn_item_index]?.quantity || 0), reason: forms[line.grn_item_index]?.reason || "" }));
    if (!current || !lines.length) return setError("Select a posted GRN and at least one item.");
    if (lines.some((line) => !(line.quantity > 0) || !line.reason.trim())) return setError("Every selected item needs a return quantity and reason.");
    try { setBusy(true); setError(""); const response = await fetch(`${API}/from-grn`, { method: "POST", headers: auth(), body: JSON.stringify({ grn_id: current.grn_id, lines, note }) }); const data = await response.json(); if (!response.ok) throw new Error(data.detail || "Could not create Return to Vendor Note."); setMessage(`${data.data.srn_no} created and stock reduced.`); setSelectedId(""); setForms({}); setNote(""); await loadGrns(); onCreated?.(); }
    catch (requestError) { setError(requestError.message || "Could not create Return to Vendor Note."); }
    finally { setBusy(false); }
  };
  return <section className="rounded-2xl border border-sky-200 bg-white p-5 shadow-sm"><div className="flex items-start gap-3"><div className="rounded-xl bg-sky-100 p-3 text-sky-700"><PackageX className="h-5 w-5"/></div><div><h3 className="font-black text-slate-900">Return after GRN (already in stock)</h3><p className="mt-1 text-sm text-slate-500">Use this only when goods were accepted and posted by GRN, then later found defective. Saving the note reduces stock immediately; then dispatch and settle it with a supplier credit note, replacement or refund.</p></div></div>{error && <p className="mt-4 rounded-lg bg-rose-50 p-3 text-sm font-semibold text-rose-700">{error}</p>}{message && <p className="mt-4 rounded-lg bg-emerald-50 p-3 text-sm font-semibold text-emerald-700">{message}</p>}<select value={selectedId} onChange={(e) => choose(e.target.value)} disabled={busy} className="mt-5 w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm font-semibold"><option value="">Select a posted GRN</option>{grns.map((row) => <option key={row.grn_id} value={row.grn_id}>{row.grn_no || "GRN"} · {row.vendor_name} · {row.stock_location}</option>)}</select>{current && <><div className="mt-4 overflow-hidden rounded-xl border border-slate-200"><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><tr><th className="p-3">Return</th><th className="p-3">Item</th><th className="p-3">Available stock</th><th className="p-3">Return quantity</th><th className="p-3">Reason</th></tr></thead><tbody>{current.lines.map((line) => { const form = forms[line.grn_item_index] || {}; return <tr key={line.grn_item_index} className="border-t border-slate-100"><td className="p-3"><input type="checkbox" checked={Boolean(form.selected)} onChange={(e) => update(line.grn_item_index, "selected", e.target.checked)} /></td><td className="p-3 font-bold text-slate-800">{line.description || line.barcode}<div className="text-xs font-normal text-slate-500">Received {qty(line.received_quantity)} · Already returned {qty(line.already_returned_quantity)}</div></td><td className="p-3 font-bold text-sky-700">{qty(line.returnable_quantity)}</td><td className="p-3"><input disabled={!form.selected} type="number" min="0.001" max={line.returnable_quantity} step="0.001" value={form.quantity || ""} onChange={(e) => update(line.grn_item_index, "quantity", e.target.value)} className="w-28 rounded-lg border border-slate-200 px-2 py-1.5 disabled:bg-slate-50"/></td><td className="p-3"><input disabled={!form.selected} value={form.reason || ""} onChange={(e) => update(line.grn_item_index, "reason", e.target.value)} className="min-w-52 rounded-lg border border-slate-200 px-2 py-1.5 disabled:bg-slate-50" placeholder="Quality defect"/></td></tr>; })}</tbody></table></div></div><textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Optional collection or inspection note" className="mt-3 min-h-20 w-full rounded-xl border border-slate-200 p-3 text-sm"/><button disabled={busy} onClick={save} className="mt-3 inline-flex items-center gap-2 rounded-xl bg-sky-700 px-4 py-2.5 text-sm font-black text-white hover:bg-sky-800 disabled:opacity-50"><Send className="h-4 w-4"/>Create Return to Vendor Note & reduce stock</button></>} {!busy && !grns.length && <p className="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No posted GRN items currently have stock available to return.</p>}</section>;
}