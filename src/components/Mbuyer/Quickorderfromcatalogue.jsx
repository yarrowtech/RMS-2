    ...
    return <div className="space-y-4">
      ...
      <section className="overflow-hidden rounded-3xl border border-teal-100 bg-gradient-to-br from-teal-800 via-teal-700 to-blue-700 p-5 text-white shadow-lg sm:p-7">
        ...
        <div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-7">{[
          ...
          [Instagram, "Instagram", vendor.instagram && (vendor.instagram.startsWith("http") ? vendor.instagram : `https://instagram.com/${vendor.instagram.replace(/^@/, "")}`)], [MapPin, "Location", vendor.location && `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(vendor.location)}`],
          ...
          <button type="button" disabled={!selected.items?.some((item) => item.direct_purchase_enabled)} onClick={jumpToDirectPurchase} className="flex min-h-16 flex-col items-center justify-center gap-1 rounded-2xl bg-emerald-500/90 px-2 text-xs font-bold ring-1 ring-white/15 transition hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-40"><PackageCheck className="h-5 w-5" />Direct Purchase</button>
        </div>
      </section>
      <section ref={galleryRef}>
        <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
          <div><h3 className="text-lg font-black text-slate-900">Product gallery</h3><p className="text-xs text-slate-500">Select an image to purchase at a fixed price or request a quotation.</p></div>
          {showDirectOnly && <button type="button" onClick={() => setShowDirectOnly(false)} className="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-3 py-1.5 text-xs font-bold text-emerald-700 hover:bg-emerald-200">Showing direct-purchase only ✕</button>}
        </div>
        {(() => {
